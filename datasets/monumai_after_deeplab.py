...
class Monumai_Deeplab(data.Dataset):
    """Monumai  Dataset.
    ...
    **Parameters:**
    ...
    def __init__(self, root, split='train', mode='fine', target_type='semantic', transform=None):
        ...
        self.images_dir = os.path.join("/home/abennetot/distillation_transparent/results/clean_deeplab", split)
    ...
    def __getitem__(self, index):
        ...
        if 'Baroque' in self.images[index]:
            target_class = 3
        elif 'Gothic' in self.images[index]:
            target_class = 1
        elif 'Hispanic' in self.images[index]:
            target_class = 0
        elif 'Renaissance' in self.images[index]:
            target_class = 2

        image = Image.open(self.images[index]).convert('RGB')

        if self.transform:
            image_final = self.transform(image)

        return image_final, target_class
